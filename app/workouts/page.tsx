...
import { 
    Dumbbell, Play, Plus, Timer, Activity, ChevronRight, Search, Save, Trash2, Zap
} from 'lucide-react';
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {filteredPrograms.map((p, idx) => (
                                <WorkoutCard key={p.id || idx} program={p} />
                            ))}
                        </div>
                    </>
                ) : (
...
function WorkoutCard({ program }: { program: any }) {
    ...
    const handleAccess = () => {
        setLoading(true);
        // This triggers the 1.5s delay + dumbbell spinner in your layout.tsx
    ...
    return (
        <div className="group bg-[#2B2B2B] rounded-2xl overflow-hidden border border-white/5 hover:border-[#F5E6B3]/30 transition-all duration-500 shadow-xl">
            <div className="relative h-64">
                ...
                <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-sm px-3 py-1 rounded border border-white/10 flex items-center gap-2">
                    <p className="text-[9px] font-bold text-[#F5E6B3] uppercase tracking-widest">{program.level || "Standard"}</p>
                    {loading && <Dumbbell className="w-3 h-3 text-[#F5E6B3] animate-spin" />}
                </div>
            </div>
            <div className="p-8">
                ...
                >
                    {loading ? "Initializing..." : "Access Training"} <Play className="w-3 h-3 fill-current" />
                </button>
            </div>
        </div>
    );
